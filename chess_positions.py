...
class CheckMate(Check):
    """
    This class finds the checkmate in the chessboard.
    """

    def __init__(self, fen_label):
        super().__init__(fen_label=fen_label)

    def is_king_checked(self, defendant):
        """
        This method identifies if the king is checked.
        """
        di, dj = self.get_piece_positions(notation=defendant)
        if len(di) == 1 and len(dj) == 1:
            di, dj = di[0], dj[0]
        ...
        if defendant.isupper():
            ...
            r, n, b, q, p = 'R', 'N', 'B', 'Q', 'P'
        r_checks = self.rook_checks_king(attacker=r, defendant=defendant)
        n_checks = self.knight_checks_king(attacker=n, defendant=defendant)
        b_checks = self.bishop_checks_king(attacker=b, defendant=defendant)
        q_checks = self.queen_checks_king(attacker=q, defendant=defendant)
        p_checks = self.pawn_checks_king(attacker=p, defendant=defendant)
        return r_checks or n_checks or b_checks or q_checks or p_checks

    def get_empty_positions(self, defendant):
        """
        This method finds if there are any empty positions around the king.
        """
        di, dj = self.get_piece_positions(notation=defendant)
        if len(di) == 1 and len(dj) == 1:
            di, dj = di[0], dj[0]
            ...
            return False
        king_moves = [(di, dj-1), (di, dj+1),
                      (di-1, dj), (di+1, dj),
                      (di-1, dj+1), (di-1, dj-1),
                      (di+1, dj-1), (di+1, dj+1)]
        positions = [move
                     for move in king_moves
                     if (move[0] not in (-1, 8) and
                         move[-1] not in (-1, 8) and
                         self.fen_matrix[move] == '1')]
        return positions

    def is_it_good_to_move(self, di, dj, i, j, d):
        """
        This method is just thinks whether to move the king or not.
        """
        self.fen_matrix[di][dj] = '1'
        self.fen_matrix[i][j] = d

    def bring_back_to_normal(self, di, dj, i, j, d):
        """
        This method brings back the normality.
        """
        self.fen_matrix[i][j] = '1'
        self.fen_matrix[di][dj] = d

    def is_king_safe(self, defendant):
        """
        This method finds all attack positions around the king.
        """
        di, dj = self.get_piece_positions(notation=defendant)
        if len(di) == 1 and len(dj) == 1:
            di, dj = di[0], dj[0]
            ...
            return False
        positions = self.get_empty_positions(defendant=defendant)
        positions_under_attack = list()
        for (i, j) in positions:
            self.is_it_good_to_move(di=di, dj=dj, i=i, j=j, d=defendant)
            if self.is_king_checked(defendant=defendant):
                positions_under_attack.append(True)
            else:
                positions_under_attack.append(False)
            self.bring_back_to_normal(di=di, dj=dj, i=i, j=j, d=defendant)
        return positions_under_attack

    def is_checkmate(self, defendant):
        """
        This method finds if there is any checkmate in the chessboard.
        """
        flag = False
        king_checked = self.is_king_checked(defendant=defendant)
        king_safe = self.is_king_safe(defendant=defendant)
        if king_checked and all(king_safe):
            flag = True
        elif king_checked and any(king_safe):
            pass
        return flag
